...
st.set_page_config(
    page_title="Welding Signal QC Playground - Prototype 4",
    layout="wide",
)
...
CL_LINE_WIDTH = 2.0
CL_COLOR = "#00FF00"
BEAD_SEPARATOR_COLOR = "rgba(255,255,0,0.25)"
HEATMAP_GRID_GAP = 1
...
def safe_filename(csv_name: str) -> str:
    return os.path.basename(csv_name)
...
def load_zip_to_data(zip_bytes: bytes) -> Tuple[Dict[str, pd.DataFrame], List[str], List[str]]:
    data = {}
...
def get_common_beads(bead_map: Dict[str, Dict[int, pd.DataFrame]]) -> List[int]:
    if not bead_map:
        return []
    bead_sets = [set(b.keys()) for b in bead_map.values()]
    if not bead_sets:
        return []
    return sorted(list(set.intersection(*bead_sets)))
...
def compute_group_assignments(sorted_files: List[str], group_size: int) -> Dict[str, int]:
    ...
    for idx, fname in enumerate(sorted_files):
        assignments[fname] = idx // group_size + 1
    return assignments
...
def compute_control_limits(
    ...
    if method == "Mean ± k·Std":
        ...
        denom = np.where((max_ref - min_ref) < 1e-12, 1.0, (max_ref - min_ref))
        norm_matrix = (step_arrays - min_ref) / denom

        aux["step_mode"] = "norm"
        aux["interval"] = interval
        aux["step_matrix"] = norm_matrix
        aux["step_center"] = np.median(norm_matrix, axis=0)
        aux["step_ucl"] = np.full(norm_matrix.shape[1], norm_high)
        aux["step_lcl"] = np.full(norm_matrix.shape[1], norm_low)
        ...
        aux["step_center"] = np.median(zmat, axis=0)
        aux["step_ucl"] = np.full(zmat.shape[1], z_high)
        aux["step_lcl"] = np.full(zmat.shape[1], -z_low)
        ...
        for fname in sorted_files:
            if fname in bead_before:
                ...
                bead_length_rows.append({
                    "file": fname,
                    "file_short": short_label(fname),
                    "bead": bead_no,
                    "length": min_len,
                    "channel": CHANNEL_LABELS[channel_idx],
                })
                ...
                bead_stat_rows.append({
                    ...
                    "min": float(np.min(bead_before[fname])),
                    "max": float(np.max(bead_before[fname])),
                    "std": float(np.std(bead_before[fname])),
                    "group": group_assignments.get(fname, 1),
                })
...
def build_display_entities(
    ...
    for f in available_files:
        raw_entities.append((short_label(f), line_map[f], f, group_assignments.get(f, 1)))
...
def add_regular_plot_traces(
    fig: go.Figure,
    raw_entities: List[Tuple[str, np.ndarray, str, int]],
    ...
    if group_size <= 1:
        for legend_name, y, _fname, _group_no in raw_entities:
    ...
    # Draw these last so they always appear at the front.
    if len(center) > 0:
        fig.add_trace(go.Scatter(
...
def add_step_plot_traces(
    ...
    if group_size <= 1:
        for i, (legend_name, _y, _fname, _group_no) in enumerate(raw_entities[:len(step_matrix)]):
            fig.add_trace(go.Scatter(
    ...
    # Draw these last so they stay on top.
    fig.add_trace(go.Scatter(
...
def get_formula_text(center_method: str, cl_method: str, adjustment_method: str, params: dict) -> str:
    lines = [f"Center method: {center_method}"]
...
def get_code_snippet(center_method: str, cl_method: str, adjustment_method: str, params: dict) -> str:
    lines = []
    lines.append("# Center")
    if center_method == "Mean":
        lines.append("center = np.mean(lines_matrix, axis=0)")
    elif center_method == "Median":
        lines.append("center = np.median(lines_matrix, axis=0)")
    elif center_method == "Trimmed Mean":
        lines.append("sorted_arr = np.sort(lines_matrix, axis=0)")
        lines.append("center = np.mean(sorted_arr[1:-1], axis=0)")
    else:
        lines.append("center = np.median(lines_matrix, axis=0)")

    lines.append("")
    lines.append("# Control Limits / QC")

    if cl_method == "Mean ± k·Std":
        k = params.get("k", 3.0)
        lines.append("mu = np.mean(lines_matrix, axis=0)")
        lines.append("sigma = np.std(lines_matrix, axis=0, ddof=1)")
        lines.append(f"ucl = mu + {k} * sigma")
        lines.append(f"lcl = mu - {k} * sigma")

    elif cl_method == "Median ± k·MAD":
        k = params.get("k", 3.0)
        lines.append("med = np.median(lines_matrix, axis=0)")
        lines.append("mad = np.median(np.abs(lines_matrix - med), axis=0)")
        lines.append(f"ucl = med + {k} * mad")
        lines.append(f"lcl = med - {k} * mad")

    elif cl_method == "Percentile Band":
        low = params.get("low_pct", 5.0)
        high = params.get("high_pct", 95.0)
        lines.append(f"ucl = np.percentile(lines_matrix, {high}, axis=0)")
        lines.append(f"lcl = np.percentile(lines_matrix, {low}, axis=0)")

    elif cl_method == "IQR Band":
        k = params.get("k", 1.5)
        lines.append("q1 = np.percentile(lines_matrix, 25, axis=0)")
        lines.append("q3 = np.percentile(lines_matrix, 75, axis=0)")
        ...
        lines.append("mu = np.mean(lines_matrix, axis=0)")
        lines.append("sigma = np.std(lines_matrix, axis=0, ddof=1)")
        lines.append(f"ucl = mu + {z} * sigma")
        lines.append(f"lcl = mu - {z} * sigma")

    elif cl_method == "Z-score Per Point":
        z = params.get("z", 3.0)
        lines.append("mu = np.mean(lines_matrix, axis=0)")
        lines.append("sigma = np.std(lines_matrix, axis=0, ddof=1)")
        lines.append("sigma = np.where(sigma < 1e-12, 1.0, sigma)")
        lines.append("zmat = (lines_matrix - mu) / sigma")
        lines.append(f"flag = np.abs(zmat) > {z}")

    elif cl_method == "Step-based Norm (M)":
        interval = params.get("step_interval", 20)
        low = params.get("norm_low", -3.0)
        high = params.get("norm_high", 4.0)
        lines.append(f"step_arrays = np.array([aggregate_for_step_1d(y, {interval}) for y in lines_matrix])")
        lines.append("min_ref = np.min(step_arrays, axis=0)")
        lines.append("max_ref = np.max(step_arrays, axis=0)")
        lines.append("denom = np.where((max_ref - min_ref) < 1e-12, 1.0, (max_ref - min_ref))")
        lines.append("norm_matrix = (step_arrays - min_ref) / denom")
        lines.append(f"flag = (norm_matrix < {low}) | (norm_matrix > {high})")
        ...
        z_high = params.get("z_high", 3.0)
        lines.append(f"step_arrays = np.array([aggregate_for_step_1d(y, {interval}) for y in lines_matrix])")
        lines.append("mu = np.median(step_arrays, axis=0)")
        lines.append("sigma = np.std(step_arrays, axis=0, ddof=1)")
        lines.append("sigma = np.where(sigma < 1e-12, 1.0, sigma)")
        lines.append("zmat = (step_arrays - mu) / sigma")
        lines.append(f"flag = (zmat < -{z_low}) | (zmat > {z_high})")

    lines.append("")
    lines.append("# Adjustment")
    if adjustment_method == "None":
        lines.append("adjusted = original.copy()")
    elif adjustment_method == "Offset Shift":
        lines.append("adjusted = y - (np.mean(y) - ref_mean)")
    elif adjustment_method == "Ratio Scaling":
        lines.append("adjusted = y * (ref_mean / y_mean)")
    elif adjustment_method == "Affine":
        lines.append("adjusted = ((y - y_mean) / y_std) * ref_std + ref_mean")
    elif adjustment_method == "Z-score Normalization":
        lines.append("adjusted = (y - y_mean) / y_std")
    elif adjustment_method == "Min-Max Normalization":
        lines.append("adjusted = (y - ymin) / (ymax - ymin)")
    elif adjustment_method == "Per-Signal Normalization":
        lines.append("adjusted = y / np.linalg.norm(y)")
...
def make_plotly_heatmap(
    ...
    y_labels = [short_label(str(v)) for v in pivot_df.index.tolist()]
    x_labels = pivot_df.columns.tolist()

    text = np.empty_like(z, dtype=object)
    hover_text = np.empty_like(z, dtype=object)

    for i, file_name in enumerate(pivot_df.index.tolist()):
        for j, bead in enumerate(pivot_df.columns.tolist()):
            val = z[i, j]
            if pd.isna(val):
                text[i, j] = ""
                hover_text[i, j] = f"File: {file_name}<br>Bead: {bead}<br>{z_label}: NaN"
            else:
                text[i, j] = f"{val:{value_format}}"
                hover_text[i, j] = f"File: {file_name}<br>Bead: {bead}<br>{z_label}: {val:{value_format}}"
    ...
    fig = go.Figure(
        data=go.Heatmap(
            ...
            xgap=HEATMAP_GRID_GAP,
            ygap=HEATMAP_GRID_GAP,
            colorscale="Viridis",
    ...
    fig.update_layout(
        ...
        plot_bgcolor="rgba(200,200,200,0.12)",
    )
    return fig
...
def make_group_histogram(
    ...
    for g in groups:
        vals = df.loc[df["group"] == g, value_col].dropna().to_numpy()
        if len(vals) == 0:
            continue
        fig.add_trace(go.Histogram(
            ...
            name=f"Group {int(g)}",
            opacity=0.55,
            hovertemplate=f"Group {int(g)}<br>{value_col}: %{{x}}<br>Count: %{{y}}<extra></extra>",
        ))
...
if uploaded_zip is not None:
    if st.session_state.zip_name != uploaded_zip.name:
        ...
        raw_data, sorted_files, column_names = load_zip_to_data(uploaded_zip.getvalue())
...
st.title("Welding Signal QC Playground - Prototype 4")
...
for channel_idx in [0, 1, 2]:
    ...
    channel_results[channel_idx] = {
        "line_map_before": line_map_before,
        "line_map_after": line_map_after,
        "bead_boundaries": bead_boundaries,
        "bead_length_table": bead_length_table,
        "bead_stat_table": bead_stat_table,
        "metadata_table": metadata_table,
        "raw_entities_before": raw_entities_before,
        "grouped_entities_before": grouped_entities_before,
        "cl_matrix_before": cl_matrix_before,
        "center_before": center_before,
        "ucl_before": ucl_before,
        "lcl_before": lcl_before,
        "point_status_before": point_status_before,
        "aux_before": aux_before,
        "raw_entities_after": raw_entities_after,
        "grouped_entities_after": grouped_entities_after,
        "cl_matrix_after": cl_matrix_after,
        "center_after": center_after,
        "ucl_after": ucl_after,
        "lcl_after": lcl_after,
        "point_status_after": point_status_after,
        "aux_after": aux_after,
    }

    if not metadata_table.empty:
        combined_metadata.append(metadata_table)

metadata_all = pd.concat(combined_metadata, ignore_index=True) if combined_metadata else pd.DataFrame()

# ============================================================
# ------------------------------ TABS -------------------------
# ============================================================

tabs = st.tabs(["Channel 1", "Channel 2", "Channel 3", "Diagnostics"])

for channel_idx, tab in zip([0, 1, 2], tabs[:3]):
    with tab:
        st.subheader(f"{CHANNEL_LABELS[channel_idx]}")

        if channel_idx not in channel_results:
            st.info("This dataset has fewer columns than expected for this channel.")
            continue

        res = channel_results[channel_idx]
        ...
        if cl_method in ["Step-based Norm (M)", "Step-based Z (M)"]:
            ...
            apply_common_layout(
                fig_before,
                f"Before Adjustment - {CHANNEL_LABELS[channel_idx]}",
                "Step Index",
                "Step-based Value"
            )
        else:
            ...
            apply_common_layout(
                fig_before,
                f"Before Adjustment - {CHANNEL_LABELS[channel_idx]}",
                "Concatenated Index Across Beads",
                "Signal Value"
            )
        ...
        if cl_method in ["Step-based Norm (M)", "Step-based Z (M)"]:
            ...
            apply_common_layout(
                fig_after,
                f"After Adjustment - {CHANNEL_LABELS[channel_idx]}",
                "Step Index",
                "Step-based Value"
            )
        else:
...
with tabs[3]:
    ...
    avg_beads_per_file = 0
    avg_bead_length = 0

    if 0 in channel_results and not channel_results[0]["bead_length_table"].empty:
        temp_len = channel_results[0]["bead_length_table"]
        avg_beads_per_file = temp_len.groupby("file")["bead"].nunique().mean()
        avg_bead_length = temp_len["length"].mean()

    d1, d2, d3, d4 = st.columns(4)
    d1.metric("Total Rows", int(total_rows))
    d2.metric("Files", len(sorted_files))
    d3.metric("Common Beads", len(common_beads))
    d4.metric("Avg Bead Length", f"{avg_bead_length:.1f}")
    ...
    channel_idx_for_heatmap = {v: k for k, v in CHANNEL_LABELS.items()}.get(heatmap_channel, 0)

    if channel_idx_for_heatmap in channel_results:
        if heatmap_mode == "Bead Length":
            df_heat = channel_results[channel_idx_for_heatmap]["bead_length_table"].copy()
            pivot = df_heat.pivot(index="file", columns="bead", values="length")
            fig_heat = make_plotly_heatmap(
                pivot,
                title=f"{heatmap_mode} Heatmap - {heatmap_channel}",
                z_label="Length",
                value_format=".0f"
            )
        else:
            df_heat = channel_results[channel_idx_for_heatmap]["bead_stat_table"].copy()
            metric_map = {
                "Signal Mean": "mean",
            ...
            value_col = metric_map[heatmap_mode]
            pivot = df_heat.pivot(index="file", columns="bead", values=value_col)
            fig_heat = make_plotly_heatmap(
                pivot,
                title=f"{heatmap_mode} Heatmap - {heatmap_channel}",
                z_label=value_col,
        ...
        st.info("No heatmap data available for the selected channel.")

    st.markdown("### Group Distribution")
    group_hist_channel = st.selectbox(
        "Histogram Channel",
        ["Channel 1", "Channel 2", "Channel 3"],
        index=0
    )
    group_hist_metric = st.selectbox(
        "Histogram Metric",
        ["mean", "median", "std", "min", "max", "length"],
        index=1
    )

    channel_idx_for_hist = {v: k for k, v in CHANNEL_LABELS.items()}.get(group_hist_channel, 0)

    if channel_idx_for_hist in channel_results:
        if group_hist_metric == "length":
            df_hist = channel_results[channel_idx_for_hist]["bead_length_table"].copy()
            if not df_hist.empty:
                df_hist["group"] = df_hist["file"].map(group_assignments)
                fig_hist = make_group_histogram(
                    df_hist,
                    "length",
                    f"Group Distribution - length - {group_hist_channel}"
                )
                st.plotly_chart(fig_hist, use_container_width=True)
            else:
                st.info("No histogram data available.")
        else:
            df_hist = channel_results[channel_idx_for_hist]["bead_stat_table"].copy()
            if not df_hist.empty:
                fig_hist = make_group_histogram(
                    df_hist,
                    group_hist_metric,
                    f"Group Distribution - {group_hist_metric} - {group_hist_channel}"
                )
                st.plotly_chart(fig_hist, use_container_width=True)
            else:
                st.info("No histogram data available.")
    else:
        st.info("No histogram data available for the selected channel.")
    ...
    code_text = get_code_snippet(center_method, cl_method, adjustment_method, cl_params)
    ...
    if metadata_all.empty:
        ...
        files_for_filter = sorted(metadata_all["file"].unique().tolist())
        beads_for_filter = sorted(metadata_all["bead"].unique().tolist())
        channels_for_filter = sorted(metadata_all["channel"].unique().tolist())

        selected_files_filter = mcol1.multiselect(
            "Filter Files",
            options=files_for_filter,
            default=[]
        )
        selected_beads_filter = mcol2.multiselect(
            "Filter Beads",
            options=beads_for_filter,
            default=[]
        )
        selected_channels_filter = mcol3.multiselect(
            "Filter Channels",
            options=channels_for_filter,
            default=[]
        )
        ...
        if selected_files_filter:
            metadata_view = metadata_view[metadata_view["file"].isin(selected_files_filter)]
        if selected_beads_filter:
            metadata_view = metadata_view[metadata_view["bead"].isin(selected_beads_filter)]
        if selected_channels_filter:
            metadata_view = metadata_view[metadata_view["channel"].isin(selected_channels_filter)]

        st.dataframe(metadata_view, use_container_width=True, height=420)

        csv_bytes = metadata_view.to_csv(index=False).encode("utf-8")
        st.download_button(
            "Download Metadata CSV",
            data=csv_bytes,
            file_name="prototype4_metadata.csv",
            mime="text/csv"
        )
